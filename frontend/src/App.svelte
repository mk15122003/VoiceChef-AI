<script>
  ...
  onMount(() => {
    ...
    // global keyboard handler for Escape in modal
...
<svelte:head>
  <script
    src="https://cdnjs.cloudflare.com/ajax/libs/socket.io-stream/0.9.1/socket.io-stream.js"
  ></script>
</svelte:head>

<div
  class="modal-outerwrapper"
  class:open={modal}
  aria-modal="true"
...
>
  ...
  >
    ...
    <div class="actions">
      <button on:click={closeModal} class="start-btn">Let's start</button>
      <button
        on:click={closeModal}
        class="secondary"
        aria-label="Close and enable mic">Enable & Close</button
      >
    </div>
    <small class="modal-tip"
      >Tip: Press the mic button above or use your voice to control the app.</small
    >
  </div>
</div>
...
<style lang="scss">
  .wrapper {
    ...
    height: 100vh;
    width: 100vw;
    background: linear-gradient(180deg, #f6f7fb 0%, #e6e9f0 100%);
  ...
  .modal-outerwrapper.open .modal-wrapper {
